...
impactFactorLevels <- finalmpactFactors %>% 
  mutate(trend = NA,
         min = NA,
         max = NA
         )
...
kable(impactFactorLevels, caption = "Description of impact factor levels")
```
...
```{r scenarioDescription, echo=FALSE}
scenarioDescription <- data.frame(Parameter = c("Market penetration", "E-bike transport", "Battery technology", "Other transport"),
                                  PhysicalUnit = c("Number of e-bikes sold annually [no. per year]", "Kilometers driven per e-bike annually [km per year]", "Share of different battery technologies used in e-bikes sold annually [% per year]", "Kilometers driven by other means of transport (annually) [km per year]"), stringsAsFactors = FALSE)


kable(scenarioDescription, caption="Information for scenarios")
```